...
function handleMsg(msg, name) {
  switch (msg.type) {
    ...
    case 'registered':
      ...
      setupWaitScreen();
      setupMenuScreen();
      showScreen('s-menu');   // vai para o menu de jogos primeiro
      break;
...
function setupMenuScreen() {
  document.getElementById('menu-greeting').textContent = 'Olá, ' + myData.name + '!';
  document.getElementById('menu-room-info').textContent = 'Sala: ' + (document.getElementById('inp-room').value || '---');
  document.getElementById('menu-avatar').style.color = myData.color;
}
